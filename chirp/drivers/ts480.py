...
class TS480Radio(KenwoodLiveRadio):
    ...
    _upper = 289
    _kenwood_split = True
    _kenwood_valid_tones = list(TS480_TONES)
    ...
    def get_features(self):
        ...
        rf.valid_tmodes = list(TS480_TMODES)
        rf.valid_tuning_steps = list(TS480_SSB_STEPS + TS480_FM_STEPS)
        rf.valid_bands = [(1000, 50000000)]
        rf.valid_skips = ["", "S"]
    ...
    def _parse_mem_spec(self, spec):
        ...
        mem.freq = int(_p4)
        mem.mode = TS480_MODES[int(_p5)]
        mem.skip = ["", "S"][int(_p6)]
        mem.tmode = TS480_TMODES[int(_p7)]
        ...
        mem.duplex = TS480_DUPLEX[int(_p12)]
        mem.offset = int(_p13)      # 9-digit
        if mem.mode in ["AM", "FM"]:
            mem.tuning_step = TS480_FM_STEPS[int(_p14)]
        else:
            mem.tuning_step = TS480_SSB_STEPS[int(_p14)]
        mem.name = _p16
    ...
    def _make_mem_spec(self, mem):
        if mem.duplex in " +-":
            duplex = util.get_dict_rev(TS480_DUPLEX, mem.duplex)
            offset = mem.offset
        ...
        if mem.mode in ["AM", "FM"]:
            step = TS480_FM_STEPS.index(mem.tuning_step)
        else:
            step = TS480_SSB_STEPS.index(mem.tuning_step)
        ...
        spec = (
            "%011i" % mem.freq,
            "%i" % (TS480_MODES.index(mem.mode)),
            "%i" % (mem.skip == "S"),
            "%i" % TS480_TMODES.index(mem.tmode),
            "%02i" % (rtone),
            "%02i" % (ctone),
            "%03i" % (dtcs),
            "0",    # REVERSE status
